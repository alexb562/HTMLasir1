...
if (formulario) {
    ...
    formulario.addEventListener("submit", (e) => {
        ...
        if (nombreEl) validarCampoTexto({ target: nombreEl }); else camposValidos.nombre = false;

        const emailEl = document.getElementById("email");
        if (emailEl) validarCampoTexto({ target: emailEl }); else camposValidos.email = false;

        const telefonoEl = document.getElementById("telefono");
        if (telefonoEl) validarCampoTexto({ target: telefonoEl }); else camposValidos.telefono = false;
